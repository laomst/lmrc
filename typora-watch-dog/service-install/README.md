...
```bash
# Unix/Linux/macOS
python src/watch_workspace.py --daemon
...
```bash
# 查看日志目录
ls -la ~/.typora-watch-dog-ext-logs/

# 实时查看当日日志
LOG_FILE=$(ls -t ~/.typora-watch-dog-ext-logs/typora-watch-dog-watch-*.log 2>/dev/null | head -1)
tail -f "$LOG_FILE"

# 查看指定日期的日志
tail -100 ~/.typora-watch-dog-ext-logs/typora-watch-dog-watch-2026-02-04.log

# 搜索最近日志中的错误
grep ERROR ~/.typora-watch-dog-ext-logs/typora-watch-dog-watch-*.log | tail -20
```
...
```bash
python src/watch_workspace.py --debounce-delay 0.5
```
